...
class DesignacaoGenerateServiceImplTest
	...
	void shouldCorrectFileInputName() {
		Assertions.assertEquals("dados-designacao.json", service.getListType().getInputFileName());
	}
	...
	void shouldGenerateListDatesEmptyException() throws IllegalAccessException {
		...
		Mockito.when(dateService.generateListDatesDesignacao(ArgumentMatchers.any(DateServiceInputDTO.class))).thenReturn(List.of());
		writeFileInputFromDto(dto);
