...
class LimpezaGenerateServiceImplTest
	...
	void shouldCorrectFileInputName() {
		Assertions.assertEquals("dados-limpeza.json", service.getListType().getInputFileName());
	}
	...
	void shouldGenerateListExceptionGeneratedListIsEmpty() throws IllegalAccessException {
		...
		Mockito.when(dateService.generateListDatesLimpeza(ArgumentMatchers.any(DateServiceInputDTO.class), ArgumentMatchers.any(Integer.class))).thenReturn(List.of());
	...
	void shouldGenerateListLayout1Success() throws IllegalAccessException, ListBuilderException {
		...
		writeFileInputFromDto(builder.withSuccess().build());
		Mockito.when(dateService.generateListDatesLimpeza(ArgumentMatchers.any(DateServiceInputDTO.class), ArgumentMatchers.any(Integer.class)))
				.thenReturn(expectedList);
		Mockito.when(groupService.generateListGroupsLimpeza(ArgumentMatchers.any(FileInputDataLimpezaDTO.class), Mockito.<ItemDateDTO>anyList(),
				ArgumentMatchers.any(Integer.class))).thenReturn(MOCK_LIST_GROUPS);
	...
	void shouldGenerateListLayout2SuccessWithAddToListException() throws IllegalAccessException, ListBuilderException {
		...
		Assertions.assertEquals("Erro ao gerar lista 'LIMPEZA': Valor 'aa-04-2022' não é uma data válida", ex.getMessage());
	}
	...
	void shouldGenerateListLayout2SuccessWithRemoveToListException()
		...
		FieldUtils.writeField(properties, "layoutLimpeza", 2, true);
		var ex = Assertions.assertThrows(ListBuilderException.class, () -> service.generateList());
		Assertions.assertEquals("Erro ao gerar lista 'LIMPEZA': Valor '12-04-aaaaa' não é uma data válida", ex.getMessage());
	}
	...
	void shouldGetLabelNoExceptionAndNoLabel() throws ListBuilderException {
		...
		Assertions.assertEquals("Terça", ret);
	}
	...
	void shouldGetLabelNoExceptionAndAndLabel() throws ListBuilderException {
		...
		Assertions.assertEquals("Terça - Após a Reunião", ret);
	}
	...
	void shouldGetLabelExceptionMessageBlankAndLabel() throws ListBuilderException {
		var dto = new ItemDateDTO(LocalDate.of(2022, 03, 01), "myMessage");
		var ret = service.getLabel(dto, true);
		Assertions.assertEquals("Terça - myMessage", ret);
	}
