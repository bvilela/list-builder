...
class VidaCristaGenerateServiceImplTest
	...
	void shouldCorrectFileInputName() {
		Assertions.assertEquals("dados-vida-crista.json", service.getListType().getInputFileName());
	}
	...
	void shouldProcessRenameOrRemoveItemRename() {
		...
		Assertions.assertEquals("Title 1 renamed", weekItemDto.getTitle());
	}
	...
	void shouldProcessRenameOrRemoveItemRemoveCaseNull() {
		...
		service.processRenameOrRemoveItem(weekItemDto, itemRename, listItemWeekToRemove);
		Assertions.assertEquals(1, listItemWeekToRemove.size());
		Assertions.assertEquals("Title 1", weekItemDto.getTitle());
	}
	...
	void shouldCheckRenameItemFromWeekMatchNameToRename() {
		...
		var listRenameItems = List.of(FileInputDataVidaCristaRenameItemDtoBuilder.create()
				...
				.build());
		service.checkRenameItemFromWeek(week, listRenameItems, weekIndex);
		Assertions.assertEquals("Title Renamed", week.getItems().get(week.getItems().size() - 1).getTitle());
	...
	void shouldCheckRenameItemFromWeekMatchNameToRemove() {
		...
		service.checkRenameItemFromWeek(week, listRenameItems, weekIndex);
		Assertions.assertEquals(initialWeekItemsSize - 1, week.getItems().size());
		Assertions.assertEquals(originalWeekItems.get(0), week.getItems().get(0));
		Assertions.assertEquals(originalWeekItems.get(1), week.getItems().get(1));
		Assertions.assertEquals(originalWeekItems.get(2), week.getItems().get(2));
		Assertions.assertEquals(originalWeekItems.get(3), week.getItems().get(3));
		Assertions.assertEquals(originalWeekItems.get(4), week.getItems().get(4));
		Assertions.assertEquals(originalWeekItems.get(5), week.getItems().get(5));
	}
	...
	void shouldCheckRenameItemFromWeekMatchNameToRemoveAndRename() {
		...
		var originalWeekItems = cloneListWithoutReference(week.getItems());
		service.checkRenameItemFromWeek(week, listRenameItems, weekIndex);
		Assertions.assertEquals(initialWeekItemsSize, originalWeekItems.size());
		Assertions.assertEquals(initialWeekItemsSize - 1, week.getItems().size());
		Assertions.assertEquals(originalWeekItems.get(0), week.getItems().get(0));
		Assertions.assertEquals(originalWeekItems.get(1), week.getItems().get(1));
		Assertions.assertEquals(originalWeekItems.get(2), week.getItems().get(2));
		...
		Assertions.assertEquals("Title Renamed", week.getItems().get(6).getTitle());
