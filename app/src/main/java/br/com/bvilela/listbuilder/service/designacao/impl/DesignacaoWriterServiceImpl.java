...
public class DesignacaoWriterServiceImpl implements DesignacaoWriterService {
	...
	public Path writerPDF(DesignacaoWriterDTO dto) throws ListBuilderException {
		try {
			...
			String fileName = FileUtils.generateOutputFileNamePDF(LIST_TYPE, dto.getPresident().get(0).getDate());
	...
	private void writeSection(PdfPTable table, List<DesignacaoWriterItemDTO> list, String section,
			int breakLine, int paddingTop) throws ListBuilderException, DocumentException {
	...
	private PdfPTable addHeaderAndTableMaster(Document document) throws ListBuilderException, DocumentException {
		pdfUtils.addImageHeader(document, LIST_TYPE);
		return pdfUtils.getTable(document, 2, LIST_TYPE);
	}
	
	private void addSubHeader(PdfPTable table, String section, int paddingTop) throws ListBuilderException {
	...
	private String getDateLabel(DesignacaoWriterItemDTO item, DayOfWeekEnum dayWeekEnum) {
		return DateUtils.formatDDMM(item.getDate()).concat(" (").concat(dayWeekEnum.getName()).concat(") ");
	}
	...
	public Path writerDocx(DesignacaoWriterDTO dto) throws ListBuilderException {
		try {
			FileUtils.createDirectories(properties.getOutputDir());
			String fileName = FileUtils.generateOutputFileNameDocx(LIST_TYPE, dto.getPresident().get(0).getDate());
			Path path = Paths.get(properties.getOutputDir(), fileName);
	...
	private void writeDocxDocument(DesignacaoWriterDTO dto, Path path) throws IOException, ListBuilderException {
	...
	private int writeSection(XWPFTable table, List<DesignacaoWriterItemDTO> list, int columnIndex, int indexRowGeneral) throws ListBuilderException {
		int countLines = 0;
		for (DesignacaoWriterItemDTO item : list) {
			XWPFTableRow row = columnIndex == 0 ? table.createRow() : table.getRow(indexRowGeneral++);
			countLines++;
	...
	private void docxAddImageSubHeader(XWPFTableCell cell, String imgName) throws ListBuilderException {
