...
@Slf4j
@Service
public class DesignationCounterService {
    ...
    private record CounterDto(
            Map<String, Long> readerGeral,
            Map<String, Long> readerBibleStudy,
            Map<String, Long> readerWatchtower,
            Map<String, Long> audioVideoGeral,
            Map<String, Long> audioVideoPrincipal,
            Map<String, Long> audioVideoHelper,
            Map<String, Long> president,
            Map<String, Long> indicator,
            Map<String, Long> microphone,
            Map<String, Long> indicatorAndMicrophone) {}

    public StringBuilder countNumberActiviesByName(DesignationWriterDTO dto) {
    ...
    private StringBuilder printCounterTable(CounterDto dto) {
        ...
        var divider =
                "|"
                        + StringUtils.leftPad(
                                "-",
                                NUMBER_COLUMNS_WITHOUT_NAMES * (COLUMN_SIZE + 1) + maxNameSize,
                                "-")
                        + "|";
    ...
    private String getLineHeader1(int maxNameSize) {
        ...
        return String.format(
                "|%s|%s|%s|%s|", labelName, subtitle1LineC1, subtitle1LineC2, subtitle1LineC3);
    }
    ...
    public Map<String, Long> count(List<DesignationWriterItemDTO> list) {
        return list.stream()
                .collect(
                        Collectors.groupingBy(
                                DesignationWriterItemDTO::getName,
                                Collectors.mapping(
                                        DesignationWriterItemDTO::getName, Collectors.counting())));
    }

    public Map<String, Long> countWithSplit(List<DesignationWriterItemDTO> list, int index) {
        return list.stream()
                .collect(
                        Collectors.groupingBy(
                                e -> e.getName().split(" e ")[index],
                                Collectors.mapping(
                                        t -> t.getName().split(" e ")[index],
                                        Collectors.counting())));
    }
