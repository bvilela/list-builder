...
import br.com.bvilela.listbuilder.util.impl.DocxWriterUtilsImpl;
...
public class DesignationWriterService {
    ...
    @SneakyThrows
    public Path writerPDF(DesignationWriterDTO dto) {
        try {
    ...
    @SneakyThrows
    private void writerPDFDocument(DesignationWriterDTO dto, Path path) {
    ...
    private void writeSectionsLine1(PdfPTable table, DesignationWriterDTO dto) {
        List<DesignationWriterItemDTO> listReader = getAllReaders(dto);
    ...
    private void writeSectionsLine2(PdfPTable table, DesignationWriterDTO dto) {
    ...
    private void writeSectionsLine3(PdfPTable table, DesignationWriterDTO dto) {
        final int marginTop = 15;
    ...
    private boolean isTextBold(DesignationWriterItemDTO item) {
        var name = item.getName().toLowerCase();
        return name.contains("congresso") || name.contains("assembleia") || name.contains("visita");
    }

    private List<DesignationWriterItemDTO> getAllReaders(DesignationWriterDTO dto) {
        List<DesignationWriterItemDTO> listReader = new ArrayList<>();
        listReader.addAll(dto.getReaderBibleStudy());
        listReader.addAll(dto.getReaderWatchtower());
        listReader.sort(Comparator.comparing(DesignationWriterItemDTO::getDate));
        return listReader;
    }

    private String getDateLabel(DesignationWriterItemDTO item, DayOfWeekEnum dayWeekEnum) {
    ...
    @SneakyThrows
    public Path writerDocx(DesignationWriterDTO dto) {
        try {
            FileUtils.createDirectories(properties.getOutputDir());
            String fileName =
    ...
    @SneakyThrows
    private void writeDocxDocument(DesignationWriterDTO dto, Path path) {
