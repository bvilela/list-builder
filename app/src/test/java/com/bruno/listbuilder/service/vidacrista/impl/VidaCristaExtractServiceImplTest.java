...
import com.bruno.listbuilder.service.vidacrista.impl.VidaCristaExtractServiceImpl;
...
class VidaCristaExtractServiceImplTest {
	...
	void shouldGetUrlMeetingWorkbook() {
		...
		Assertions.assertEquals("https://www.jw.org/pt/biblioteca/jw-apostila-do-mes/janeiro-fevereiro-2022-mwb/", url);
	}
	...
	void shouldGetUrlMeetingWorkbookCase3() {
		...
		Assertions.assertEquals("https://www.jw.org/pt/biblioteca/jw-apostila-do-mes/maio-junho-2022-mwb/", url);
	}
	...
	void shouldGetUrlMeetingWorkbookCase5() {
		...
		Assertions.assertEquals("https://www.jw.org/pt/biblioteca/jw-apostila-do-mes/janeiro-fevereiro-2023-mwb/", url);
	}
	...
	void shouldExtractWeeksBySite() throws ListBuilderException, IOException {
		...
		int i = 0;
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.PRESIDENT, "Presidente da Reunião");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.READ_OF_WEEK, "1 SAMUEL 27-29");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Cântico 71 e oração");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.NO_PARTICIPANTS, "Comentários iniciais");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.LABEL, "TESOUROS DA PALAVRA DE DEUS");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "\"A estratégia de guerra de Davi\"");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Joias espirituais");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Leitura da Bíblia");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.LABEL, "FAÇA SEU MELHOR NO MINISTÉRIO");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Vídeo da primeira conversa");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Primeira conversa — designação 1");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Primeira conversa — designação 2");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.LABEL, "NOSSA VIDA CRISTÃ");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.NO_PARTICIPANTS, "Cântico 129");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Firmes apesar de oposição");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Estudo bíblico de congregação");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.NO_PARTICIPANTS, "Comentários finais");
		checkWeekItem(week.getItems().get(i++), VidaCristaExtractItemType.WITH_PARTICIPANTS, "Cântico 94 e oração");
	}

	@Test
	void shouldExtractWeeksBySiteAnotherSite() throws ListBuilderException, IOException {
		var url = "https://www.google.com.br";
		var ex = Assertions.assertThrows(ListBuilderException.class, () -> service.extractWeeksBySite(url));
		Assertions.assertEquals("Erro ao ler Cabeçalho do site", ex.getMessage());
	}

	@Test
	void shouldExtractMonthsAndConvertToDatesException() {
		var ex = Assertions.assertThrows(ListBuilderException.class, () -> service.extractMonthsAndConvertToDates("abc", 2022));
	...
	void shouldExtractMonthsAndConvertToDatesTwoMonthsSuccess() throws ListBuilderException {
		...
		Assertions.assertEquals(List.of(LocalDate.of(2022, 9, 26), LocalDate.of(2022, 10, 2)), list);
	}
	...
	void shouldExtractMonthsAndConvertToDatesTwoMonthsTwoYearCase1Success() throws ListBuilderException {
		...
		Assertions.assertEquals(List.of(LocalDate.of(2022, 12, 26), LocalDate.of(2023, 1, 1)), list);
	}
